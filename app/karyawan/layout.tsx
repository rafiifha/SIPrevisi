...
export default function KaryawanLayout({ children }: { children: React.ReactNode }) {
    ...
    const navItems = [
        { href: '/karyawan', label: 'Dashboard' },
        { href: '/karyawan/barang', label: 'Data Barang' },
        { href: '/karyawan/transaksi', label: 'Transaksi' },
    ]
                    ...
                    <div className="flex items-center justify-between h-16">
                            ...
                            <div className="flex space-x-1">
                                {navItems.map((item) => (
                                    <Link
                                        key={item.href}
                                        href={item.href}
                                        className={`px-4 py-2 rounded-lg text-sm font-medium transition ${pathname === item.href
                        ...
                        <div className="flex items-center space-x-4">
                            <span className="text-slate-300 text-sm">
                                {user?.username} <span className="text-slate-500">(Karyawan)</span>
                            </span>
                            <button
